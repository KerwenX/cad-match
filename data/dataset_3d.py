...
class ROCA(data.Dataset):
    def __init__(self, config):
        ...
        print_log(f'[DATASET] sample out {self.sample_points_num} points', logger='ROCA')
        print_log(f'[DATASET] Open file {self.data_list_file}', logger='ROCA')
        with open(self.data_list_file, 'r') as f:
            lines = f.readlines()
        if self.whole:
            with open(test_data_list_file, 'r') as f:
                test_lines = f.readlines()
            print_log(f'[DATASET] Open file {test_data_list_file}', logger='ROCA')
            lines = test_lines + lines
        self.file_list = []
        for line in lines:
            line = line.strip()
        ...
        print_log(f'[DATASET] {len(self.file_list)} instances were loaded', logger='ROCA')
    ...
    def __getitem__(self, idx):
        ...
        try:
            if os.path.splitext(picked_image_addr)[1]=='.npy':
                depth = np.load(picked_image_addr).astype(np.uint8)
                depth = depth.reshape(*depth.shape,1)
                image = Image.fromarray(np.concatenate((depth,depth,depth),axis=-1),mode='RGB')
            else:
                image = pil_loader(picked_image_addr)
            image = self.train_transform(image)
        except:
