...
class ULIPWithImageLoss(nn.Module):
    ...
    def forward(self, outputs):
        ...
        loss = F.cross_entropy(logits_per_image_pc, self.labels)
        ...
        with torch.no_grad():
            ...
            pred = torch.argmax(logits_per_image_pc, dim=-1)
            correct = pred.eq(self.labels).sum()
            # pc_image_acc = 100 * correct / local_batch_size
            image_pc_acc = 100 * correct / local_batch_size

        return {'loss': loss, 'ulip_loss': loss, 'ulip_image_pc_acc': image_pc_acc}
